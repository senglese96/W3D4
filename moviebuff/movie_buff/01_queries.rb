...
def harrison_ford
  ...
  # :actors is an association that goes through :castings which also automatically joins the :castings joins table
  # so we don't have to explicitly do it ourselves dope
...
def biggest_cast
  ...
  Movie.joins(:castings).select(:id, :title).group('movies.id').order("COUNT(*) DESC").limit(3)
end
